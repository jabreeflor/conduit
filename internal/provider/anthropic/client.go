...
func parseSSE(r interface {
	...
	// Track in-progress blocks by index so partial_json deltas accumulate
	// onto the right tool_use.input.
	type pending struct {
		block     ContentBlock
	...
	for scanner.Scan() {
		...
		var env struct {
			...
			// message_delta carries stop_reason inside delta
		}
		if err := json.Unmarshal([]byte(payload), &env); err != nil {
			continue // tolerate the occasional non-JSON keepalive
		}
