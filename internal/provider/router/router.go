...
import (
	...
	conduiterrors "github.com/jabreeflor/conduit/internal/platform/errors"
)
...
type ContextAssembler interface {
	Assemble(assembler.Request) assembler.Result
}

// OptimizationSink receives context assembler transparency reports.
type OptimizationSink interface {
	RecordContextOptimization(context.Context, assembler.Summary) error
}
...
func (r *Router) assembleContext(ctx context.Context, req Request) Request {
	...
	result := r.assembler.Assemble(assembler.Request{
		Query: req.Prompt,
...
func routerContextItems(req Request) []assembler.Item {
	items := []assembler.Item{{
		ID:       "prompt",
		Category: assembler.CategoryRecent,
		Content:  req.Prompt,
		Pinned:   true,
	}}
	for i, input := range req.Inputs {
		item := assembler.Item{
			...
			Category: assembler.CategoryMultimodal,
			Path:     input.Ref,
