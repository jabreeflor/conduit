...
func TestRouterAssemblesContextBeforeProviderCall(t *testing.T) {
	...
	r, err := New(
		cfg,
		[]Provider{openai},
		WithContextAssembler(assembler.New()),
...
type fakeOptimizationSink struct {
	summaries []assembler.Summary
}

func (s *fakeOptimizationSink) RecordContextOptimization(_ context.Context, summary assembler.Summary) error {
